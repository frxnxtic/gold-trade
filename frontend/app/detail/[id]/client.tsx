        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
          ...
          <div className="col-span-1 md:col-span-2 flex flex-col items-center relative">
            ...
            <div className="relative w-full rounded-xl overflow-hidden shadow-lg">
              ...
              <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/70 hover:bg-black/90 transition text-yellow w-10 h-10 flex items-center justify-center rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="yellow">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
              </button>
              <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/70 hover:bg-black/90 transition text-yellow w-10 h-10 flex items-center justify-center rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="yellow">
            ...
            <div className="text-center md:text-left">
                ...
                <div className="text-gray-300 mt-6 text-base max-w-lg mx-auto">
                    <table className="w-full mb-2">
                        ...
                        {
                            ["Obývacia izba s kuchyňou", "Izba", "Kúpeľňa", "Chodba", "WC", "Komora"].map(type =>
                    ...
                    {flat.popis.some(item => item.includes("Obytná plocha")) && (
                        <div className="flex justify-between items-center mt-2">
                            <span className="text-white font-bold">Obytná plocha:</span>
                            <span><span>{flat.popis.find(item => item.includes("Obytná plocha"))?.split(':')[1] ?? ''}</span></span>
                        </div>
                    )}
                    <hr className="border-yellow-400 my-3" />
                    {/* Balkón и Пивница */}
                    ...
                    {flat.popis
                        .filter(item =>
                            !["Obývacia izba s kuchyňou", "Izba", "Kúpeľňa", "Chodba", "WC", "Komora", "Obytná plocha", "Balkón", "Pivnica"].some(key => item.includes(key))
                        )
                        .map((item, i) => (
